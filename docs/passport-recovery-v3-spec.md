...
3. Implement the dedicated production wrapper and generated verifier, keeping the developer verifier in an isolated test-only package/profile.
...
14. **Partially completed:** the old root authorization map/API and the callable generic
    `recover(...)` selector have been removed from source and the regenerated issuer ABI. Before
    production, complete the security review, decide and document the non-passport lifecycle for
    the still-existing rootless `RecoveryNote` storage/issuance shape, remove any remaining
    passport-inapplicable rootless surface, recreate deployments from scratch, and rerun the full
    production-profile regression and adversarial suite.
