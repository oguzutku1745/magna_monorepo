...
11. **Completed through the official mobile Docker refresh on 2026-09-06:** the digest-pinned Aztec 5.1.0 stack rebuilds application and proof artifacts, deploys one Anvil with the managed local clock, validates independent developer registry roots, and starts the API/apps only after successful bootstrap. The existing gateway delay elapses in historical bootstrap time; subsequent checkpoints wait for real slots. The official mobile run passed private/EVM mutation and replay rejection, canonical Inbox ingestion, Ghost funding and proven successful recovery at L2 block 45. Public receipts and browser-observed destination state are recorded separately in [the September 6 evidence](evidence/recovery-v3-mobile-2026-09-06.md). Earlier registry-negative and independent-root evidence remains in the dated Gate B-dev and bootstrap records. The clean Docker command builds and loads its replacement image before removing the previous disposable chain. SDK publication does not require resetting that chain.
