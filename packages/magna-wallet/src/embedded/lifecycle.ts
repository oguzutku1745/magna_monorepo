...
function stringifyAddress(address: { toString(): string } | string): string {
...
export async function syncEmbeddedWalletPxeIfAvailable(wallet: EmbeddedWallet): Promise<void> {
...
export async function ensureAccountManagerRegistered(
...
export type DeploymentFundingOptions = {
  localTestAccountIndex?: number;
  fromAddress?: string;
};
...
export async function ensureAccountManagerDeployed(
  wallet: EmbeddedWallet,
  accountManager: AccountManager,
...
export async function restoreEmbeddedAccountManager(
  wallet: EmbeddedWallet,
  address: string,
  alias?: string,
