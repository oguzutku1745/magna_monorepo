...
| `npm run test:issuance-uniqueness:e2e` | Against a running local network, deploy a fresh issuer and prove that a second rooted Passport A2 issuance for the same scoped identity is rejected onchain. |
...
- **Instagram after passport recovery:** Instagram V2 is a rootless credential, so Passport Root
  Recovery V3 does not copy its private notes into the recovered wallet. After opening the recovered
  wallet, the holder selects a fresh signed Instagram security email and issues Instagram again.
  The email and handle remain in the browser; Magna API receives only the client-generated proof and
  its public outputs.
