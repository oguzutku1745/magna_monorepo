...
for (const directory of ['magna-core', 'magna-client']) {
  ...
  copyFileSync(resolve('LICENSE'), resolve(cwd, 'LICENSE'));
  ...
  assert.equal(readFileSync(resolve(cwd, 'LICENSE'), 'utf8'), readFileSync(resolve('LICENSE'), 'utf8'));
  assert.ok(packed.files.some(file => file.path === 'dist/index.js'));
  assert.ok(packed.files.some(file => file.path === 'dist/index.d.ts'));
  for (const { path } of packed.files) {
    assert.match(path, /^(package\.json|README\.md|LICENSE|dist\/[\w-]+\.(js|d\.ts))$/, `Unexpected package file: ${path}`);
