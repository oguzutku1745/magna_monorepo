...
function usage() {
  return `Usage:
  node ./scripts/fill-env-from-local-deploy.mjs [options]
...
Options:
  ...
  --aztec-node-url <url>             Optional browser-facing Aztec node URL override
  --l1-rpc-url <url>                 Optional browser-facing L1 RPC URL override
...
function main() {
  ...
  const templatePath = resolve(repoRoot, String(args.template ?? "apps/magna-management/.env.example"));
  const manifestPath = resolve(repoRoot, String(args.manifest ?? `deployments/${networkName}.json`));
  const outPath = resolve(repoRoot, String(args.out ?? "apps/magna-management/.env"));
  ...
  const aztecNodeUrl = requireString(
    preferString(args.aztecNodeUrl, manifest?.endpoints?.aztecNodeUrl),
    "aztec node URL",
    "arguments or manifest endpoints.aztecNodeUrl",
  );
  ...
  const orchestratorAddress =
    ...
    adminAddress;
  const l1RpcUrl =
    (typeof args.l1RpcUrl === "string" && args.l1RpcUrl.trim()) ||
    manifestL1RpcUrl ||
  ...
  const l1BuyerPrivateKey =
    ...
    "";
  const recoveryV3RelayerPrivateKey =
    (typeof args.recoveryV3RelayerPrivateKey === "string" && args.recoveryV3RelayerPrivateKey.trim()) ||
    existingOutPairs.get("VITE_MAGNA_RECOVERY_V3_RELAYER_PRIVATE_KEY") ||
    templatePairs.get("VITE_MAGNA_RECOVERY_V3_RELAYER_PRIVATE_KEY") ||
    "";
  const localFaucetPrivateKey =
    (typeof args.localFaucetPrivateKey === "string" && args.localFaucetPrivateKey.trim()) ||
    existingOutPairs.get("VITE_MAGNA_LOCAL_FAUCET_PRIVATE_KEY") ||
    templatePairs.get("VITE_MAGNA_LOCAL_FAUCET_PRIVATE_KEY") ||
    "";
  ...
  merged.set("VITE_MAGNA_RECOVERY_V3_RELAYER_PRIVATE_KEY", recoveryV3RelayerPrivateKey);
  merged.set("VITE_MAGNA_LOCAL_FAUCET_PRIVATE_KEY", localFaucetPrivateKey);
