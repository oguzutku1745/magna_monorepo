...
function usage() {
  return `Usage:
  node ./scripts/bootstrap-local-apps.mjs [options]
...
Options:
  ...
  --public-aztec-node-url <url>       Optional browser-facing node URL (Docker/local proxy)
  --public-l1-rpc-url <url>           Optional browser-facing L1 URL (Docker/local proxy)
...
async function runRetriedStep(label, work) {
  let attempt = 1;
  while (true) {
    console.info(`[app-bootstrap] ${label}${attempt > 1 ? ` (retry=${attempt - 1})` : ""}`);
    try {
      return await work();
    } catch (error) {
      const details = errorDetails(error);
      ...
      console.info(`[app-bootstrap] transient local-network tx error: ${details}`);
      await sleep(250);
      attempt += 1;
    }
  }
...
async function syncWalletPxeAfterWarp(label, wallet) {
  ...
  console.info(`[app-bootstrap] ${label} syncing PXE after L1 warp`);
  await retryUntil(
    async () => {
      try {
...
async function warpForwardSeconds({ l1RpcUrls, l1Mnemonic, label, seconds, wallet }) {
  ...
  console.info(
    `[app-bootstrap] ${label} warped time ` +
      `(from_ts=${currentTimestamp.toString()} to_ts=${targetTimestamp.toString()})`,
  );
  await syncWalletPxeAfterWarp(label, wallet);
}

async function mineThreeL2BlocksForBridgeIngestion(l2NudgeToken, orchestrator, label = "bridge ingestion") {
  ...
  await runRetriedStep(`${label} L2 nudge #3`, async () => {
    return await l2NudgeToken.methods.mint_to_public(nudgeRecipient, 1n).send({ from: orchestrator });
  });
}
...
function runNodeScript(scriptPath, args, description) {
  console.info(`[app-bootstrap] ${description}`);
...
function runRightsDeployScript(scriptPath, args, description, manifestPath) {
  ...
  if (manifestHasRightsStack(manifestPath)) {
    ...
    console.info(
      `[app-bootstrap] ${description} ended with ${exitDescription} after writing a complete manifest; continuing.`,
    );
    return;
  }
  throw new Error(`${description} failed with exit code ${result.status ?? 1}`);
...
function syncAppBootstrapOutputs({
  repoRoot,
  networkName,
  manifestPath,
  ...
  if (managementEnvOutPath) {
    runNodeScript(
      resolve(repoRoot, "scripts/fill-env-from-local-deploy.mjs"),
      [
        "--network-name",
        networkName,
        "--manifest",
...
async function ensureL1PaymentTokenAddress({ existingAddress, l1Client }) {
  if (existingAddress) {
    try {
      ...
      console.info(
        `[app-bootstrap] existing L1 payment token address is not usable on current network; ` +
...
async function main() {
  ...
  const l1RpcUrls = String(
    ...
    .map(url => url.trim())
    .filter(Boolean);
  const publicAztecNodeUrl = String(args.publicAztecNodeUrl ?? nodeUrl);
  const publicL1RpcUrl = String(args.publicL1RpcUrl ?? l1RpcUrls[0]);
  ...
  console.info(`[app-bootstrap] aztec node ready (${nodeUrl})`);
  ...
  try {
    ...
    const l2RightsPurchase = MagnaRightsPurchaseL2Contract.at(
      AztecAddress.fromStringUnsafe(purchaseAdapterAddress),
      wallet,
    );
    const l2PaymentToken = TokenContract.at(AztecAddress.fromStringUnsafe(paymentTokenAddress), wallet);
    ...
    syncAppBootstrapOutputs({
      ...
      description: "checkpoint deployed application stack",
      publicAztecNodeUrl,
      publicL1RpcUrl,
      verificationApiUrl,
      recoveryV3RelayerPrivateKey,
    ...
    await mineThreeL2BlocksForBridgeIngestion(
      l2PaymentToken,
      orchestrator,
    ...
    syncAppBootstrapOutputs({
      ...
      referenceDappConsumerAddress: referenceDappConsumer.address.toString(),
      txHashes,
      description: "finalize deployed application stack",
      publicAztecNodeUrl,
      publicL1RpcUrl,
      verificationApiUrl,
      recoveryV3RelayerPrivateKey,
...
main().catch(error => {
  console.error(`[app-bootstrap] ${errorDetails(error)}`);
  process.exit(1);
});
