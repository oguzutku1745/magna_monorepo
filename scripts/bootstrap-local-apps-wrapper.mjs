...
function outputsAreComplete({ manifestPath, managementEnvOutPath, referenceDappEnvOutPath }) {
  try {
    if (
    ...
    return managementEnvMatchesManifest && referenceDappEnvIsReady;
  } catch {
    return false;
  }
}
...
const result = await runWorker(resolve(here, "bootstrap-local-apps.mjs"), process.argv.slice(2));
...
if (
  outputsAreComplete({ manifestPath, managementEnvOutPath, referenceDappEnvOutPath })
) {
  console.info("[app-bootstrap] verified complete local outputs after Aztec worker teardown.");
  process.exit(0);
}

console.error(`[app-bootstrap] bootstrap failed with ${exitDescription(result)} before complete outputs were written.`);
